...
class User {
	...
	public function getProjects() {
		...
		while ($row = mysql_fetch_assoc($result)) {
			array_push($projects, array("project_id" => $row["project_id"], "title" => stripslashes($row["title"]), "slug" => $row["slug"], "author" => stripslashes($row["author"]), "role" => $row["role"], "completed" => $row["completed"], "total" => $row["total"], "available_pages" => $row["available_pages"], "proof_percentage" => $row["proof_percentage"], "num_proofs" => $row["num_proofs"]));
		}

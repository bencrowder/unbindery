...
$text_username = $_GET["user"];				// the user who proofed the text

$username = Alibaba::getUsername();		// the current user (admin or owner)

if (!$item_id || !$project_slug || !$text_username) {
...
$item = new Item($db);
$item->load($item_id, $project_slug, $text_username);
	...
	<div id="controls_container" class="review">
		<div id="controls">
			...
			<ul id="controls_right">
				...
				<li><span id="finished_review_button" class="button">Finish Review</span></li>
			</ul>
		</div>
	</div>

	<div id="main">
		<h2><?php echo $item->title; ?> (proofed by <span id="review_username"><?php echo $text_username; ?>)</span></h2>
